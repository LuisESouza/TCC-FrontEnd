...
// Funcao para abrir o modo de edição apenas do exercicio específico selecionado
const editExercices = (exerciseId) => {
  const treino = exercises.value.find((t) =>
    t.exercicios.some((ex) => ex.treino_exercicio_id === exerciseId)
  );
  const exercise = treino?.exercicios.find((ex) => ex.treino_exercicio_id === exerciseId);
  if (exercise && treino) {
    newExercise.value = {
      id_treino: treino.id,
      id_exercicio: exercise.treino_exercicio_id,
    ...
    console.warn('Exercise or training not found for ID:', exerciseId);
  }
};
...
// Funcao para salvar as alteracoes feitas em um exercicio específico
const saveExercices = async () => {
  try {
    const url = import.meta.env.VITE_URL_PUT_TREINO;
    await api.apiPut(url, newExercise.value);
  } catch (error) {
    console.log(error);
  } finally {
    editingState.value[newExercise.value.id_exercicio] = false;
    isEditing.value = false;
  }
};
const submit = (treino) => {
              ...
              <div class="exercise-content">
                <div class="d-flex justify-content-between">
                  ...
                  <button class="btn-edit" 
                          @click="editExercices(exercise.treino_exercicio_id)" 
                          v-if="!editingState[exercise.treino_exercicio_id]">
                    ...
                    <button class="btn-cancel" @click="closeEdit(exercise.treino_exercicio_id)" v-if="editingState[exercise.treino_exercicio_id]">
                      <i class="fas fa-x"></i>
                    </button>
                  </div>
                </div>
                ...
                <div class="d-flex justify-content-between">
                  <div class="exercise-details" v-show="!editingState[exercise.treino_exercicio_id]">
                  ...
                  <div class="container-input" v-if="editingState[exercise.treino_exercicio_id]">
